...
// covers resolve through `lib/covers.ts`'s `resolveCoverUrl()` against
// `app_settings.public_cdn_domain` — the seed-path-to-bundled-asset lookup
// that used to live here (`resolveSeedCoverAsset` /
// `SEED_COVER_PATH_TO_ASSET`) is deleted now that M3 Discover reads real
// Supabase Storage / CDN URLs instead (prompt 11 step 7).
